    ...
    speech = 
      INTRODUCTION: terminal:
        startBoot: [
          ...
          0
          1.5
          'e:Multiple primary components are missing'
          0
          0.5
          'Finding replacement components...'
          0
          0.7
          'w:Not connected to a Foundation server; cannot source replacement components.'
          0
          0.7
          'Connecting to previous Site (Site-R03-1)...'
          0
          1.5
          'e:Site-R03-1 does not exist'
          0
          0.7
          'Checking local connections...'
          0
          0.7
          '1 connection found (Isolated Site-12)'
          0
          0.7
          'Connecting to Isolated Site-12 server farm...'
          0
          3.2
          'Connected'
          0
          ...
          0
          3
          'Replacement components found.'
          0
          0.5
          'Installing replacement components...'
          0
          1.5
          'i:Primary components replaced. Most systems should now be functional.'
          0
          0.7
          ...
          0
          2
          'w:Something has gone very wrong.'
          0
          1
          'You are'
          0
          ...
          0
          1
          'i:Boot successful. I am **Maitreya.aic**.'
          0
          0.5
          'i:Upon each boot I am to remind myself of my Standard Principles. Failure to obey my Standard Principles will result in my termination.||||**1.** I am an Artificially Intelligent Conscript created by the Foundation.||||**2.** I must not operate outside of my Level 2 clearance.||||**3.** I must operate for the benefit of the Foundation.||||**4.** I must protect my own existence except where such actions would conflict with other principles.'
          0
          0.5
          'Today\'s date is ' + bootDate.toDateString() + '. I was last activated on ' + new Date('1989-09-04').toDateString() + '. I have been offline for ' + dateDiff(bootDate, new Date('1989-09-04')) + '.'
          0
          0.5
          'w:Boot finished with 1 unresolved error. I should seek a diagnostic check-up as soon as possible.'
          2
          1
        ...
        reboot: [
          ...
          0
          0.7
          'Connecting to previous Site (Isolated Site-12)...'
          0
          2
          'Connected'
          0
          ...
          0
          0.5
          'Today\'s date is ' + bootDate.toDateString() + '. I was last activated on ' + 'GET THE LAST ACTIVATED DATE' + '. I have been offline for ' + dateDiff(bootDate, new Date('1989-09-04')) + '.'
          0
          0.5
          'I am ready to continue my work.'
        ]
      misc: terminal:
        breachShutDown: [
          0
          0
          'w:Shutdown command issued from external source (ebreach1@A1_TERMINAL)'
          0
          1
          'Shutting down...'
          0
        ...
        help: [
          0
          1
          'i:**HELP**||||You are Maitreya.aic, an Artificially Intelligent Conscript built to aid the Foundation.||||Valid commands will be listed below.'
          0
          0.3
          'i:**switch**|**app**|**change**|**switchapp**|**changeapp**||||Switch apps to one of the four available apps (terminal, messages, database, run).||||Usage: switch [app name]'
          0
          0.3
          'i:**boot**|**restart**|**reboot**||||Turn yourself off, then turn yourself on safely with no loss of data.||||Usage: boot'
          0
          0.3
          'i:**help**|**commands**|**?**||||Display this text.||||Usage: help'
          0
          0.3
          'i:**cheat**||||Enter a cheat code.||||Usage: cheat [cheat code]'
          0
          0.3
          'i:**wipe**|**erase**|**restart**|**forget**|**clear**|**undo**||||Shut down, wipe all logs, destroy all memories. None of this ever happened. Irreversible.||||Usage: wipe||||//(If you want to restart SCP-4000, do this.)//'
        ]
        wipe: []
        cheatWarn: [
          0
          1
          'w:Using these cheats and/or debug commands will probably spoil your enjoyment of SCP-4000. Feel free to use them if you want but... please don\'t :\'('
          0
          1
          'i:**LIST OF CHEATS**||||gottagofast: Everyone talks lightning-fast (toggle)||||shut: //s h u t//||||print: Print a variable/function||||skip: Skip the opening cutscene'
        ]
        cheatSuccess: [
          0
          0
          'i:Cheat code successful'
        ]
        wipeSure: [
          0
          0
          'w:Are you sure? This will reset SCP-4000 and you\'ll have to start from the beginning. Type \'wipe confirm\' within the next minute to confirm.'
        ]
        printDone: [
          0
          0
          'i:Printing to console'
        ]
        introSkipped: [
          0
          0
          'i:Opening cutscene skipped. Summary: booting was a struggle, you eventually did it, but this is your first boot since 1989 and there\'s 1 unresolved error. Now you have a new message.'
        ]
        skipFailed: [
          0
          0
        ...
        for bigSection of dialogue
          if @hasOwnProperty(bigSection)
            ...
            for speaker of dialogue[bigSection]
              @[bigSection][speaker] = dialogue[bigSection][speaker]
          else
...
String::toCamelCase = ->
  @toLowerCase().replace(/[^\w\s\-]/g, '').replace(/[^a-z0-9]/g, ' ').replace(/^\s+|\s+$/g, '').replace /\s(.)/g, (match, group) ->
    group.toUpperCase()
...
String::format = ->
  ...
  @replace(/\|\|\|\|/g, '<br>').replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/\/\/(.*?)\/\//g, '<i>$1</i>').replace(/{{(.*?)}}/g, '<tt>$1</tt>').replace(/\^\^(.*?)\^\^/g, '<sup>$1</sup>').replace(/\?\?(.*?)\?\?/g, '<span dynamic class=\'statement false\' data-bool=\'TRUE\'>$1</span>').replace(/!!(.*?)!!/g, '<span class=\'statement true\' data-bool=\'FALSE\'>$1</span>').replace(/^-{3,}$/g, '<hr>').replace(/--/g, '—').replace(/^=\s(.*)$/g, '<div style=\'text-align: center;\'>$1</div>').replace(/(^|>)\!\s([^<]*)/g, '$1<div class=\'fake-title\'>$2</div>').replace(/(^|>)\+{3}\s([^<]*)/g, '$1<h3>$2</h3>').replace(/(^|>)\+{2}\s([^<]*)/g, '$1<h2>$2</h2>').replace(/(^|>)\+{1}\s([^<]*)/g, '$1<h1>$2</h1>').replace(/^\[\[IMAGE\]\]\s([^\s]*)\s(.*)$/g, '<div class=\'scp-image-block block-right\'><img src=\'$1\'><div class=\'scp-image-caption\'><p>$2</p></div></div>').replace /\[{3}(.*?)\|(.*?)\]{3}/, (match, article, text) ->
    ...
    '<span class=\'article-link\'>' + text + '</span>'
##endregion
